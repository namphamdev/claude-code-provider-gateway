...
| `packages/daemon/src/proxy/providers/commandcode/index.ts` | Custom provider with request conversion, catalog handling, and NDJSON/SSE stream transformation. | Split new behavior into helper modules where possible and keep tests close to the stream state machine. |
